...
def verificar_arquivo_github(caminho_relativo):
    # O Streamlit precisa dessas chaves configuradas em 'Settings > Secrets'
    ...
    # Limpa espaços extras que podem vir do copiar/colar
    repo = repo.strip()
    
    # Lista de tentativas para ignorar erros de acentuação na pasta
    tentativas = [
        caminho_relativo,
...
# --- LAYOUT EM COLUNAS ---
col_principal, col_espaco, col_direita = st.columns([3, 0.2, 1])
...
with col_principal:
    ...
    if not df_clientes.empty:
        ...
        st.error("⚠️ Base de clientes não encontrada nas pastas locais.")
        selecao = None
    ...
    if selecao:
        ...
        # Caminho oficial que você confirmou que está no GitHub
