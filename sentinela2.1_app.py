...
def limpar_central_arquivos():
    # Chaves específicas que serão resetadas no corpo principal
    chaves_corpo = ["xml_up", "ge", "ae", "gs", "as", "analise_concluida"]
    for key in chaves_corpo:
        if key in st.session_state:
            st.session_state[key] = None if "up" in key or "ge" in key or "ae" in key or "gs" in key or "as" in key else False
    
    # Resetando os file_uploaders especificamente através de suas chaves
    st.rerun()
...
with st.sidebar:
    ...
    if not df_clientes.empty:
        ...
        # Key única e fixa para a sidebar
        selecao = st.selectbox("Escolha a empresa", [""] + opcoes, key="sb_empresa")
    else: 
    ...
    if selecao:
        ...
        regime = st.selectbox("Escolha o Regime Fiscal", ["", "Lucro Real", "Lucro Presumido", "Simples Nacional", "MEI"], key="sb_regime")
        
        st.markdown("### 🏗️ Passo 3: Segmento")
        tipo_ipi = st.selectbox("Escolha o Segmento", ["", "Comércio (Não gera IPI)", "Indústria", "Equiparado à Indústria"], key="sb_segmento")
        
        st.markdown("### 🛡️ Passo 4: RET")
        is_ret = st.toggle("Habilitar MG (RET)", key="sb_ret")
        ...
        if localizar_base_dados(cod_cliente):
        ...
        st.download_button("📥 Modelo Bases", criar_gabarito(), "modelo_gabarito.xlsx", use_container_width=True, type="primary", key="btn_mdl")
...
with col_r:
    ...
    # Botão de reset com chave única
    if st.button("🔄 REINICIAR ARQUIVOS", use_container_width=True, key="btn_reset_main"):
...
if selecao:
    ...
    with c1:
        xmls = st.file_uploader("📁 XML (ZIP)", type=['zip', 'xml'], accept_multiple_files=True, key="xml_up")
    with c2:
    ...
    with col_btn:
        if st.button("🚀 INICIAR ANÁLISE", key="btn_run"):
    ...
    if st.session_state.get('analise_concluida'):
        ...
        st.download_button("💾 BAIXAR RELATÓRIO FINAL", st.session_state['analise_concluida'], f"Sentinela_{cod_cliente}_v2.1.xlsx", use_container_width=True, key="btn_dl_res")
else:
