...
# --- MOTOR DE EXTRAÇÃO XML (MAPEAMENTO INTEGRAL DE TAGS) ---
...
def gerar_excel_final(df_xe, df_xs, cod_cliente, writer, regime, is_ret, ae=None, as_f=None, df_base_emp=None, modo=None):
    ...
    try:
        ...
        st.error(f"⚠️ Falha ao carregar módulos especialistas: {e}")
        return

    # Execução das Auditorias (Apenas se houver Saídas)
    if not df_xs.empty:
        processar_icms(df_xs, writer, cod_cliente, df_xe, df_base_emp, modo) # Aba 1
        try: processar_ipi(df_xs, writer, cod_cliente)                      # Aba 2
        except: pass
        try: processar_pc(df_xs, writer, cod_cliente, regime)               # Aba 3
        except: pass
        try: processar_difal(df_xs, writer)                                 # Aba 4
        except: pass
        try: gerar_resumo_uf(df_xs, writer, df_xe)                          # Aba 5
        except: pass
